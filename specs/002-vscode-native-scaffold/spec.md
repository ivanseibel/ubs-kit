...
**Feature Branch**: `002-vscode-native-scaffold`  
**Created**: 2026-02-07  
**Status**: Draft  
...
- Q: Should the system migrate legacy .ubs content into the new native locations? → A: No; only scaffold native locations and report if legacy .ubs exists.
- Q: How should conflicts be handled during scaffolding? → A: Abort the entire scaffold if any conflict is detected (no files written).
- Q: What scope of validation is required beyond UBS structure? → A: Validate only UBS structure and required files/paths; skip agent/skill metadata.
- Q: How should scaffolded skills be organized? → A: Multiple focused skills (template, guidelines/checklist, examples).
- Q: How should assistant handoffs behave? → A: Handoffs prefill the next prompt but require user confirmation to run.
...
- The workspace contains legacy .ubs content that should be reported but not
  migrated.
...
- **FR-002a**: The scaffolded skills MUST be split into focused skills for the
  template, guidelines/checklist, and examples.
...
- **FR-003a**: Handoffs MUST prefill the next prompt and MUST NOT auto-run
...
- **FR-004a**: Validation MUST cover UBS structure and required files/paths but
  MUST NOT validate agent or skill metadata.
- **FR-005**: The system MUST detect conflicts and MUST NOT overwrite user
  content without explicit override.
- **FR-005a**: If any conflict is detected, the system MUST abort the scaffold
  and write no files.
...
- **FR-008**: If legacy .ubs content exists, the system MUST report it and MUST
  NOT migrate, delete, or alter it.
